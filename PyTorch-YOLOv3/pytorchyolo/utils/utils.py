...
def print_environment_info():
    ...
    try:
        print("Current Version: {}".format(subprocess.check_output(['poetry', 'version'], stderr=subprocess.DEVNULL).decode('ascii').strip()))
    except (subprocess.CalledProcessError, FileNotFoundError):
        print("Not using the poetry package")

    # Print commit hash if possible
    try:
        print("Current Commit Hash: {}".format(subprocess.check_output(['git', 'rev-parse', '--short', 'HEAD'], stderr=subprocess.DEVNULL).decode('ascii').strip()))
    except (subprocess.CalledProcessError, FileNotFoundError):
